...
export default function AllCourses({data}){
    ...
    <div className='lg:w-5/6 h-full'>
            ...
            {data.map(item=>
            <div className='flex flex-col lg:p-1 lg:border-t-2 border-gray-400'>
                <ul className='flex lg:gap-6'><li>Curso: <b>{item.curso.codigo}</b> - {item.curso.nome}</li> <li>CargaHorária: <b>{item.curso.cargaHoraria}</b></li></ul>
                <ul className='flex lg:gap-6'><li>Matrícula: <b>{item.numeroMatricula ? item.numeroMatricula : 'N/D'}</b></li><li>Início do Curso: <b>{item.inicioCurso ? new Date(item.inicioCurso).toLocaleDateString('pt-BR'):'Aguardando'}</b></li>

  ...
  return (
    <div className={`${!courses.length && !openCart > 0 ? 'invisible' : false} bg-[#D1D5DB] lg:fixed lg:right-20 lg:mt-0 md:mt-20 h-fit p-4 rounded-xl md:w-1/2 w-full lg:w-1/4 md`}>
        ...
        <div className='mt-5 flex flex-col md:flex-row justify-around'>
          <button onClick={()=> {setCoursesSelected([]);setOpenCart(false)}} className='bg-red-600 text-white xl:text-lg font-semibold rounded-lg p-2 md:p-1 lg:p-2'>Limpar Carrinho</button>
          ...
          <button className='bg-yellow-400 xl:text-lg md:ml-5 mt-3 md:mt-0 font-semibold rounded-lg p-2 md:p-1 lg:p-2'>Finalizar Compra</button>
        </div>
    </div>
  )
}

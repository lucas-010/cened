...
export default function MenuDropdown({collapse, visible, id}) {
  return (
    <div>
        ...
        <ul className={`bg-[#c8c9cd] bottom-32 left-44 text-center ${!(id === 2) ? 'hidden' : visible ? 'block' : 'hidden'} md:hover:block lg:hover:block fixed ${collapse ? 'lg:top-12' : 'lg:top-40'} rounded-b-lg mt-1 p-4 lg:right-5 lg:bottom-auto lg:left-auto`}>
            <Link to='login'>

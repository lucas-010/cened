...
export default function MenuDropdown({collapse, visible, id}) {
  return (
    <div>
        <ul className={`bg-[#c8c9cd] top-52 left-44 ${!(id === 1) ? 'hidden' : visible ? 'block' : 'hidden'} md:hover:block lg:hover:block fixed ${collapse ? 'lg:top-12 lg:mt-2' : 'lg:top-32 mt-11'} md:top-56 lg:rounded-t-none rounded-t-lg rounded-b-lg mt-1 p-4 md:left-48 lg:left-64`}>
            <Link to='passoapasso'>
        ...
        <ul className={`bg-[#c8c9cd] bottom-32 left-44 text-center ${!(id === 2) ? 'hidden' : visible ? 'block' : 'hidden'} md:hover:block lg:hover:block fixed ${collapse ? 'lg:top-12 lg:mt-2' : 'lg:top-32 mt-11'} rounded-b-lg mt-1 p-4 lg:right-1 lg:bottom-auto lg:left-auto`}>
            <Link to='login'>

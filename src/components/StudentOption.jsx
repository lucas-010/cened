...
export default function StudentOption({id ,text, selected, setSelected}) {
  ...
  return (
      <ConditionalLink to='/' condition={id===10}>
			<button onClick={()=>setSelected(id)} className={`${selected ===id ? 'bg-gray-400' : ''} font-bold flex gap-5 items-end bg-gray-300 border-2 border-white  hover:bg-gray-400 transition-colors p-4 rounded-lg w-full`}>
